...
public class Student
{
     ...
     *         absence}
     */
    public static List<String> getPresenceStatistic(String studentName, String curGroupName, String mainGroupName,
	    String dates)
    {
	...
	String querry1 = "SELECT COUNT(v.visitingId) " + "FROM Students AS s INNER JOIN Groups AS g ON "
		+ "s.groupId = g.groupId INNER JOIN Visitings as v ON v.studentId = s.studentId " + "WHERE s.name = '"
		+ studentName + "' AND s.birthdate IN (SELECT s2.birthdate "
	...
	String querry3 = "SELECT AVG(v.mark)" + "FROM Students AS s INNER JOIN Groups AS g ON "
		+ "s.groupId = g.groupId INNER JOIN Visitings as v ON v.studentId = s.studentId " + "WHERE s.name = '"
		+ studentName + "' AND s.birthdate IN (SELECT s2.birthdate "
		+ "FROM Students AS s2 INNER JOIN Groups AS g2 ON s2.groupId = g2.groupId AND g2.name = '"
	...
	if (dates != "")
	{
	    ...
	    querry1 += " AND v.date >= '" + splittedDates[0] + "' AND v.date <= '" + splittedDates[1] + "'";
	    querry2 += " AND v.date >= '" + splittedDates[0] + "' AND v.date <= '" + splittedDates[1] + "'";
	    querry3 += " AND v.date >= '" + splittedDates[0] + "' AND v.date <= '" + splittedDates[1] + "'";
	}
	querry1 += ";";
	querry2 += ";";
	querry3 += ";";
	...
	String querry = "SELECT s.name, birthdate, ismale FROM Students AS s, Groups AS g WHERE " + "g.name = '"
		+ groupName + "' AND s.groupid = g.groupid ORDER BY s.name;";
	...
	String querry = "SELECT s.name, birthdate, ismale FROM Students AS s "
		+ "INNER JOIN Groups AS g ON s.groupid = g.groupid WHERE " + "g.name = '" + groupName
	...
	String querry = "INSERT INTO Students (name, birthdate, ismale, groupid) " + "VALUES ('" + newStudent.getName()
		+ "', '" + newStudent.getBirthday() + "', " + ((newStudent.isMale()) ? 1 : 0)
    ...
    {
	String querry = "DELETE FROM Students WHERE groupid IN " + "(SELECT g.groupid FROM Groups AS g WHERE g.name = '"
		+ groupName + "') AND name = '" + studentName + "';";
	...
	String querry = "UPDATE Students SET name = '" + newStudent.getName() + "' , birthdate = '"
		+ newStudent.getBirthday() + "', ismale = " + (newStudent.isMale() ? 1 : 0) + " WHERE name = '"
		+ oldName + "' AND groupid IN (SELECT groupid FROM Groups WHERE name = '" + groupName + "');";
