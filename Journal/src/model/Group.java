...
public class Group {
	...
	public static Group getGroup(String groupName)
	{
		...
		String querry = "SELECT name, daysOfWeek, startDate, finishDate FROM Groups WHERE "
				+ "name = '" + groupName + "'";;
		DB db = DB.conn();
		List<String> strList = db.select(querry);
		for(String str : strList)
	 ...
	 * @param groupName - name of 1 of his group
	 * @return list of names of his groups 
	 * (search students with same name and birthdays in all groups)
	 */
	public static List<String> getGroupNames(String studentName, String groupName)
	{
		...
		String querry = "SELECT g.name FROM Groups AS g "
				+ "INNER JOIN Students AS s ON g.groupId = s.groupId WHERE s.name = '"
				+ studentName + "' AND s.birthdate IN (SELECT s2.birthdate FROM Students as s2 "
	 ...
	 * @return list of DayOfWeek singletons. Ex: {MONDAY, WEDNESDAY, THURSDAY, SUNDAY}
	 */
	...
	/**
	 * Method transform util.List of time.DaysOfWeek into text representation of days of work 
	 * @param list - of DayOfWeek singletons. Ex: {MONDAY, WEDNESDAY, THURSDAY, SUNDAY}
	 * @return String, containing numbers of days of week, separeted by " ". Ex: "1 3 4 7".
	 */
	...
	public static Group createNewGroup(String name)
	{
		...
		String querry = "INSERT INTO Groups (name, daysOfWeek, startDate, finishDate) VALUES ('"
				+ name + "', '', '" + newGroup.getStartDate().toString() + "', '"
	...
	public static Group changeGroup(String oldName, String newName, LocalDate startDate,
			LocalDate finishDate, List<DayOfWeek> daysOfWeek)
	{
		Group newGroup = new Group(newName, daysOfWeek, startDate, finishDate);
		String querry = "UPDATE Groups SET "
				+ "name = '" + newName + "', daysOfWeek = '"
				+ Group.parseFromDaysOfWeek(daysOfWeek) + "', startDate = '"
				+ startDate.toString() + "', finishDate = '"
				+ finishDate.toString() + "' WHERE name = '"
				+ oldName + "';";
		DB db = DB.conn();
		db.executeUpdate(querry);
		return newGroup;
