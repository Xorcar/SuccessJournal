...
public class GroupFullWinController
{
	...
	if (dataList.size() >= 1)
	{
	    for (int m = 0; m < (dataList.get(0).size() - 4) / 3; m++)
	    {
		...
		columnM.setCellValueFactory(param -> {
		    ...
		    String cellStr = param.getValue().get(3 * index - 3).equals("null") ? "0"
			    : param.getValue().get(3 * index - 3);
	...
	columnTotPres.setCellValueFactory(param -> {
	    int index = (table.getColumns().size() - 4) * 3 + 3;
	    String cellStr = param.getValue().get(index);
	    return new SimpleStringProperty(cellStr);
	});
	...
	dteTo.setOnAction(new EventHandler<ActionEvent>()
	{
	    @Override
	    public void handle(ActionEvent e)
	    {
	...
	String selectedStudentName = table.getSelectionModel().getSelectedItems().get(0).get(1);
    ...
    {
	// TODO end
	System.out.println("Open new editing window: info about group " + groupName + " at date ");
